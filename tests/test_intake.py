...
class IntakePackageTests(unittest.TestCase):
    ...
    def test_a_download_writes_a_package_matching_the_schema(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            # The Archivist opens source_filename relative to the package, so it
            # names the quarantined file; the publisher's name stays in the URI.
            self.assertEqual(package["source_filename"], "DCSA_FCL_Orientation_Handbook_20260828.pdf")
            self.assertTrue((packages[0].parent / package["source_filename"]).is_file())
